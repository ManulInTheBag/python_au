...
def last_comment_time(pull, author):
    ...
    last_comment = "9999-00-00T00:00:00Z"
    for comment in all_comments:
        if comment['user']['login'] == author:
                if CHECKER in comment['body']:
                                last_comment = comment['created_at']
    return last_comment

def was_not_checked(pull, author):
        pepe = True
        all_comments = requests.get(pull['review_comments_url'], headers=prepare_headers()).json()
        for comment in all_comments:
                if comment['user']['login'] == author:
...
def check_pull(pull, checker):
        message = ''
        if was_not_checked(pull, checker):
                message = message + prepare_check_message(pull['title'], False)
        for commit in get_all_commits(pull).json():
                if last_comment_time(pull, checker) < get_commit_time(commit):
